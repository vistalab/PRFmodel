function [result,noiseVals,sds] = pmNoise2AccPrec(DT, accorprec, varargin)
...
result = [];
sds    = [];
for sc=1:length(noiseVals)
    % This is what it should be:
    objectiveValue = unique(DT.synth.sMaj);
    
    % Create the x value and the distributions
    x = DT.tool.sMaj(DT.noise2sig == noiseVals(sc));
