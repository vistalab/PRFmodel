function result = twoChansmodel(param, stimulus, t)
...
adapt_exp = exp(-(1:(60000/(1000/fs))) / (x.tau_ae/(1000/fs)));
...
for k = 1 : size(stimulus, 1)
    ...
    adapts = code_exp_decay(stimulus', onsets,offsets,adapt_exp,fs);
    adapt_acts(k,:) = convCut(nrfS, adapts, length(stimulus));


    % transient with sigmoid -------------------------------------------
    predT = convCut(nrfT, stimulus(k, :), length(stimulus));
%     predT(k, :) = normMax(predT(k, :));
    predTs(k, :) = tch_sigmoid(predT(k, :), x.lambda_p, x.kappa_p, x.lambda_p, x.kappa_n); 
...
%     adapt_acts(k,:) = x.weight * adapt_acts(k,:);
