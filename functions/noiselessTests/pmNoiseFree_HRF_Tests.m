function [compTable, tSeries, synthDT] = pmNoiseFree_HRF_Tests(prfImplementation, varargin)
% Starting from the perfect solution checked in pmNoiseFreeTests, use this
% function to introduce variations in the hrf to evaluate its effect
% 
% Syntax:
%    prfImplementation = 'popeye' %'afni' 'popeye' % 'vista' % 'aprf';
%    [compTable, tSeries] = pmNoiseFree_HRF_Tests(prfImplementation);
%  
% Brief description:
...
switch prfimplementation
    case {'aprf','analyzeprf'}
        ...
        HRF(1).params.stimDuration              = 0.5;
        HRF(2).Type                             = 'canonical';
        HRF(2).params.stimDuration              = 1;
        HRF(3).Type                             = 'canonical';
        HRF(3).params.stimDuration              = 2;
        HRF(4).Type                             = 'canonical';
        HRF(4).params.stimDuration              = 4;
    ...
    case {'vista','mrvista','vistasoft'}
        ...
        % COMBINE_PARAMETERS.Stimulus.ResizedHorz = [101];
...
switch prfimplementation
    ...
    case {'vista','mrvista','vistasoft'}
        results      = pmModelFit(synthDT,'vistasoft', ...
            ...
            'detrend', 0);  %  We need to remove it otherwise it will find an average HRF for all of them
    case {'popeye'}
