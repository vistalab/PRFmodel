function  [img,coords] = makeMontage(imgCube, sliceList, fileName, numAcross, backVal,firstrowcol)
...
if(~exist('firstrowcol', 'var')) firstrowcol = true; end
if(size(imgCube,4)==3)
...
if(isempty(backVal)) backVal = 0.5; end
...
for ii = 1:length(sliceList)
    ...
    if firstrowcol
        % Make first row and col same color as background to diferentiate images
        tmp = img(y+1:y+r,x+1:x+c);
        tmp(1,:) = ones(size(tmp(1,:))) * backVal;
        tmp(:,1) = ones(size(tmp(:,1))) * backVal;
        img(y+1:y+r,x+1:x+c) = tmp;
    end
    coords(ii,:) = [x+1,y+1];
end
